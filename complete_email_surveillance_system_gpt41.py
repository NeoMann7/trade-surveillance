...
def load_comprehensive_emails():
    """Load comprehensive dealing emails with full body content."""
    try:
        ...
        print(f"📧 Loaded {len(emails)} emails from comprehensive file")
        return emails
    except Exception as e:
        print(f"❌ Error loading comprehensive emails: {e}")
        return None

def process_emails_with_gpt41():
    """Process all emails with GPT-4.1-mini and full email body."""
    
    # Load comprehensive emails
    emails = load_comprehensive_emails()
    if not emails:
        return None
    
    print(f"🤖 Starting AI analysis with GPT-4.1-mini...")
